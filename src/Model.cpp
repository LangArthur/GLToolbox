...
namespace GLTools
{
    ...
    std::vector<Texture> Model::loadMaterialTextures(aiMaterial *material, aiTextureType aiType, Texture::TextureType type)
    {
        Texture::LoadingParams params {
        ...
        for (unsigned int i = 0; i < material->GetTextureCount(aiType); i++)
        {
            ...
            if (m_cachedTextures.find(path.C_Str()) == m_cachedTextures.end())
            {
                textures.emplace_back((m_directory + "/" + path.C_Str()).c_str(), type, params);
                m_cachedTextures.emplace(path.C_Str(), textures.back());
            }
            else
            {
                textures.push_back(m_cachedTextures.at(path.C_Str()));
            }
        }
        return textures;
    }
