...
// global variables
constexpr auto WINDOW_HEIGHT = 720.0f;
constexpr auto WINDOW_WIDTH = 1080.0f;
...
{
    GLTools::Window window(WINDOW_WIDTH, WINDOW_HEIGHT, "Chap 3");
    if (glewInit() != GLEW_OK)
    {
    ...
    glm::vec3 dirLightColor(glm::vec3(0.5f, 0.1f, 0.5f));
    dirLight.ambient = dirLightColor * glm::vec3(0.05f);
    dirLight.diffuse = dirLightColor * glm::vec3(0.1f);
    dirLight.specular = dirLightColor * glm::vec3(0.0f);
    ...
    while (!glfwWindowShouldClose(window.data()))
    {
        ...
        pointLight.position.x = pointLightPosition.x * pointLightRadius + glm::cos((float)glfwGetTime());
        pointLight.position.y = pointLightPosition.y * pointLightRadius + glm::sin((float)glfwGetTime());
        pointLight.render(colorShader, "pointLights[" + std::to_string(0) + "]");
        dirLight.render(colorShader, "dirLight");
